...
function getSearchDates(daysAhead = 30, los = 1) {
  ...
  return {
    ...
    checkInDate: `${ymd(ci)}T15:00:00.000Z`,
    localCheckInDate: ymd(ci), localCheckoutDate: ymd(co),
  };
}
  ...
  return {
    ...
    PricingSummaryRequest: {
      ...
      pricing: {
        ...
        features: { crossOutRate: false, isMSE: true, isRPM2Included: true, maxSuggestions: 0, enableRichContentOffer: true },
...
async function fetchCitySearch(cityId, { page = 1, daysAhead = 30, cookie = process.env.AGODA_COOKIE } = {}) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Agoda ${res.status} ${res.statusText} (anti-bot? 쿠키 필요할 수 있음). ${txt.slice(0, 200)}`);
  }
...
function nearestLandmark(landmarks = {}) {
  ...
  if (station) return { kind: 'station', name: station.landmarkName, m: station.distanceInM };
  const top = pick(landmarks.topLandmark);
  if (top) return { kind: 'landmark', name: top.landmarkName, m: top.distanceInM };
  const beach = pick(landmarks.beach);
  if (beach) return { kind: 'beach', name: beach.landmarkName, m: beach.distanceInM };
  return null;
}
...
module.exports = { fetchCitySearch, mapProperty, nearestLandmark, parsePriceKRW, priceBand, priceText, bayesian, getSearchDates, buildVariables };
